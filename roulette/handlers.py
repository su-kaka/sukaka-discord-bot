    ...
    async def on_message(message: discord.Message) -> None:
        ...
        if content == RULES_KEYWORD:
            rules = (
                ...
                "🙏 **乞讨**：发起乞讨，别人施舍你 10 点（施舍者扣 12 点）。\n"
                "⚔️ **决斗**：双方押上额度最少者的全部额度，赢家获得 80%（20% 销毁），需 ≥ 10 点。\n"
                "🧧 **红包**：押额度的 10%（最少 10 点），80% 随机分给所有参与的人（20% 销毁）。\n"
                ...
                "🌒 **黑市**：随机上架 5 种卡牌，价格 100-500 点随机、库存 1-5 件随机，点击购买；卖光后自动上新，不卖唯一道具，背包道具需「收藏家」才能叠加数量。\n"
                ...
                "🧧🧧 **大红包**：机器人每 5 分钟发 500 点，最多 8 人抢，全员分完整池。\n"
                "**买彩票**：10 点一张，5% 概率赢走全部奖池，未中奖 8 点进奖池（2 点销毁），奖池基础 50 点。\n"
                "🧧 **来财**：发送「来财」随机掉落 0-100 点额度（30% 概率掉 0 点，10% 概率反而被扣 1-100 点），每次触发后进入 30-180 秒随机冷却。\n"
                "🏆 **排行榜**：展示活动额度前十用户。"
            )
