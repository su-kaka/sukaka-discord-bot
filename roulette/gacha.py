...
def set_divinity_holder(discord_id: int) -> None:
    """设置神性唯一持有者（覆盖旧持有者）。"""
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            """
            INSERT INTO divinity_holder (id, discord_id, created_at)
            VALUES (1, ?, ?)
            ON CONFLICT(id) DO UPDATE SET
                discord_id = excluded.discord_id,
                created_at = excluded.created_at
...
def get_divinity_holder() -> Optional[int]:
    """查询当前神性持有者，无持有者返回 None。"""
    with sqlite3.connect(DB_PATH) as conn:
        row = conn.execute(
            "SELECT discord_id FROM divinity_holder WHERE id = 1"
        ).fetchone()
    return row[0] if row else None


def has_divinity(discord_id: int) -> bool:
    """是否持有神性（唯一道具）。"""
    return get_divinity_holder() == discord_id


def clear_divinity_holder() -> None:
    """神力耗尽：销毁神性（清除持有者记录）。"""
    with sqlite3.connect(DB_PATH) as conn:
...
async def handle_gacha(
    ...
    if card_key == "wishingpool":
        ...
        view.message = await message.channel.send(
            f"🎴 {message.author.mention} 消耗 {cost} 点抽卡……\n"
            f"🌠 **{name}**！{desc}\n"
            f"⏳ 请在 {GACHA_WISHING_TIMEOUT_SECONDS} 秒内从下方三个栏目（唯一道具/即时生效卡/背包道具卡）中选择一张，超时视为放弃。",
            view=view,
        )
        return
...
async def _settle_error(
    ...
    effect_text = f"💥 额度从 **{quota} 点** 重置为 **{new_quota} 点**！"
    if announce:
        await message.channel.send(
...
async def _settle_inflation(message: discord.Message, announce: bool = True) -> Optional[str]:
    """通货膨胀：若存在存款 > INFLATION_MIN_BALANCE 的用户，所有人存款减半。

    announce=False 时不播报，返回效果文本（黑市购买时由调用方播报）。"""
    accounts = get_all_accounts_with_min_balance(1)
    if not accounts:
        effect_text = "💸 银行空无一人，通货膨胀无效果。"
        if announce:
            await message.channel.send(effect_text)
        return effect_text

    has_rich = any(balance > INFLATION_MIN_BALANCE for _, balance in accounts)
    if not has_rich:
        effect_text = f"💸 银行没有存款超过 {INFLATION_MIN_BALANCE} 点的用户，通货膨胀无效果。"
        if announce:
            await message.channel.send(effect_text)
        return effect_text
    ...
    if announce:
        await message.channel.send(
            f"💸 {message.author.mention} 抽中 **通货膨胀**！{effect_text}"
        )
    return effect_text
...
async def _settle_sellout(
    ...
    effect_text = (
        f"🏷️ 卖掉 {len(selected)} 种道具，获得 **{total} 点**额度：\n" + "\n".join(sold_lines)
    )
    if announce:
        await message.channel.send(
...
async def _settle_selfdestruct(
    ...
    if announce:
        await message.channel.send(
            f"💥 {message.author.mention} 抽中 **自爆**！\n{effect_text}"
        )
...
class WishPoolView(discord.ui.View):
    ...
    选项按「唯一道具」/「即时生效卡」/「背包道具卡」拆成三个下拉菜单：
    Discord 限制单个下拉菜单最多 25 个选项，全部塞进一个会被 400 拒绝。
    唯一道具选择后立即替换持有者；即时生效卡选择后立即结算；持续型卡牌放入背包。
    60 秒内未选择视为放弃。
    """
    ...
    def __init__(self, message: discord.Message, client: httpx.AsyncClient) -> None:
        ...
        unique_options: list[discord.SelectOption] = []
    ...
    @discord.ui.select(placeholder="👑 唯一道具卡……", row=0)
    async def select_unique(self, interaction: discord.Interaction, select: discord.ui.Select) -> None:
        await self._pick_card(interaction, select.values[0])

    @discord.ui.select(placeholder="⚡ 即时生效卡……", row=1)
    async def select_instant(self, interaction: discord.Interaction, select: discord.ui.Select) -> None:
        await self._pick_card(interaction, select.values[0])

    @discord.ui.select(placeholder="🎒 背包道具卡……", row=2)
    ...
    async def _pick_card(self, interaction: discord.Interaction, card_key: str) -> None:
        name, desc, _ = CARD_POOL[card_key]
        self.selected = True
        self.select_unique.disabled = True
