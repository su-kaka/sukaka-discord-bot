...
- 「来财」关键词随机掉落活动额度：在指定频道发送「来财」随机掉落 0-50 点活动额度，单用户冷却，掉落时频道内提醒并自动删除
...
- 掉落功能监听指定频道的用户发言（自动过滤机器人消息），发送「来财」关键词触发：每次随机掉落 0-50 点（30% 概率掉 0 点，10% 概率变为扣减事件）；无论掉落几点都会进入 30-180 秒的随机冷却，冷却期间不再掉落。掉落成功时在频道内 @用户 提醒，10 秒后自动删除。冷却记录存于 SQLite，重启后仍有效。
