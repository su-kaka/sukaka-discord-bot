...
- **SQLite 本地库**（项目根目录）：`gacha.db`（卡牌效果/下线状态/身体交换）、`bank.db`（银行存款/抢劫冷却）、`lottery.db`（奖池）、`quota_drops.db`（掉落冷却）。
...
1. **单一 `on_message` 监听**：整个项目只有 `roulette/handlers.py` 注册了一个 `@bot.event on_message`，且第一行就按频道 ID 过滤。其他功能要么是斜杠命令（channel_admin），要么是定时任务（carousel、大红包、抢银行邀请）。**不要再注册第二个 `on_message`**，否则后注册的会覆盖先注册的（discord.py 的 `@bot.event` 是单播语义）。
